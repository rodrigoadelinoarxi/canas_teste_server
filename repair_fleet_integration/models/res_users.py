...
class ResUsers(models.Model):
    ...
    restrict_locations = fields.Boolean(default=True)
    ...
    def update_restrict(self):
        restrict_group = self.env.ref('repair_fleet_integration.stock_restrictions_group')
    ...
    def tgl_restrict(self):
        ...
        if self.stock_location_ids:
            ...
            pick_types = self.env['stock.picking.type'].sudo().search([
                '|', '|', '|', ('default_location_src_id', 'in', self.stock_location_ids.ids),
                ('default_location_src_id.location_id', 'in', self.stock_location_ids.ids),
                ('default_location_dest_id', 'in', self.stock_location_ids.ids),
                ('default_location_dest_id.location_id', 'in', self.stock_location_ids.ids),
            ])
            current_group.write({'users': [(4, self.id)]})
            self.groups_id = [(4, restrict_group.id)]
